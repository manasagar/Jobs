...
public interface JobDetailsRepository extends JpaRepository<JobDetails,Integer> {
    ...
    @Query(nativeQuery = true,value="select * from job where company like concat(?1,'%') ")
    public Page<JobDetails> findFilteredJobs(String company, String location, Pageable pageable);
}
